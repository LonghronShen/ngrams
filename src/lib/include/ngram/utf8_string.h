...
public:
  ...
  utf8_string() : buffer(NULL), strLength(0), bufferLength(0) {}

  // string constructors, mark them explicit to force us to find unnecessary
  // casting
  // Construct a string from a null-terminated character array.
  // explicit
  utf8_string(const char *cstring);

  // Construct a string from a char
  // explicit
  utf8_string(const char ch);

  // construct a string from an integer
  utf8_string(int value);
   ...
   */
  utf8_string(size_t len, const char chr = ' ');

  /**
   * Construct a string with substring of str from index start with specified
   * length
   * @param str new string
   * @param start - start index
   * @param len - len of the substring
   */
  utf8_string(const utf8_string &str, size_t start, size_t len);

  /**
   * Copy constructor
   */
  utf8_string(const utf8_string &copy);
  ...
  // string  operators
  const utf8_string &operator=(const char *content);
  const utf8_string &operator=(const utf8_string &copy);
  const utf8_string &operator=(const char ch);

  bool operator==(const utf8_string &str) const {
    return this->strLength == str.strLength && compare(str.c_str()) == 0;
  }

  bool operator==(const char *str) const { return this->compare(str) == 0; }
  ...
  utf8_string &operator+=(const char *suffix) {
    assert(suffix);
    this->append(suffix);
    return *this;
  }

  utf8_string &operator+=(int single) {
    this->append(single);
    return *this;
  }
  ...
  /**
   * New size computation. It is simplistic right now : it returns twice the
   * amount we need
   */
   ...
   */
  bool isEmpty() const { return this->length() ? false : true; }
   ...
   */
  inline bool operator!(void) const { return this->isNull(); }

  /**
   * return true if the characters is NULL
   */
  bool isNull() const { return this->buffer == NULL; }
  ...
  utf8_string &append(const char *suffix) {
    return this->append(suffix, strlen(suffix));
  }

  utf8_string &append(const char *str, size_t len);

  // append function for another string
  utf8_string &append(const utf8_string &suffix) {
    return this->append(suffix.c_str(), suffix.length());
  }

  // append for a single char.
  utf8_string &append(int c);
   ...
   */
  void insert(size_t start, const utf8_string &str);
   ...
   * The substring starts at a specified character position, and extracted to
   * the end
   */
  inline utf8_string substring(size_t start) const {
   ...
   */
  inline utf8_string substring(size_t start, size_t len) const {
    return utf8_string(*this, start, len);
  };
   ...
   */
  inline utf8_string &trim() { return this->trim(" "); }

  /**
   * Removes all occurrences of a set of characters specified in an array from
   * both the start and end of this instance.
   *
   ...
   * trimChars are removed from both the  start and end. If trimChars is a null
   * reference, white space characters are removed instead.
   */

  inline utf8_string &trim(const char *trimChars) {
   ...
   */
  inline utf8_string &trimEnd() { return this->trimEnd(" "); }
   ...
   * @param trimChars - An char to be removed or a null reference.
   * @return The String that remains after all occurrences of the characters in
   * trimChars are removed from the end. If trimChars is a null reference, white
   * space characters are removed instead.
   */
  utf8_string &trimEnd(const char *trimChars);

  /**
   * Removes all space from the start of this instance.
   */
  inline utf8_string &trimStart() { return trimStart(" "); }
   ...
   * trimChars are removed from the beginning. If trimChars is a null reference,
   * white space characters are removed instead.
   */
  utf8_string &trimStart(const char *trimChars);

  /**
   * Removes a specified number of characters from this instance beginning at a
   * specified position.
   *
   * @param startIndex - The position in this instance to begin deleting
   * characters.
  ...
  utf8_string &replace(const char *oldValue, const char *newValue) {
   ...
   */
  utf8_string &replace(size_t startIndex, size_t len, const char *text,
                       size_t count);
   ...
   */
  utf8_string &replace(size_t startIndex, size_t len,
  ...
  /**
   * Reports the index of the first occurrence of a String with length len, or
   * one or more characters, within this instance.
   * @param chars - NULL terminated char string
   * @return The index position of value if that character is found, or -1 if it
   * is not.
   */
  int indexOf(const utf8_string &str) const { return indexOf(str.c_str()); }
   ...
   * is not.
   */
  int indexOf(const char *chars) const {
    return chars ? indexOf(chars, 0) : -1;
  }
   ...
   * is not.
   */
  int indexOf(const char *chars, size_t startIndex) const {
    return indexOf(chars, startIndex, chars ? strlen(chars) : 0);
  }
   ...
   *
   * @param pattern - NULL terminated pattern string ( can also be called
   * keyword )
   * @param len - length of the pattern string
   * @param startIndex - the start position of the string for searching.
   * @return The index position of value if pattern string is found, or -1 if it
   * is not.
   */
...
private:
  ...
  inline int compare(const char *text, size_t len = 0, size_t index = 0) const {
    ...
    return index > this->strLength
               ? -1
               : len ? strncmp(this->buffer + index, text, len)
                     : strcmp(this->buffer + index, text);
  }
};

ostream &operator<<(ostream &out, const utf8_string &str); // Output
istream &operator>>(istream &in, utf8_string &str);        // Input
istream &getline(istream &in, utf8_string &str);           // Read line

utf8_string operator+(const utf8_string &s1, const char c2);
utf8_string operator+(const char c1, const utf8_string &s2);
