...
public:
  ...
  struct NgramToken {
    NgramToken() {}
    NgramToken(utf8_string &newNgram, NgramValue &newValue) {
      ngram = newNgram;
      value = newValue;
    }
    // Copy constructor
    ...
    */
    ...
    /**
     * use following operators if we need to order the ngrams by frequency and
     * ngram
     */
    /*
    bool operator>( const NgramToken & ngramToken ) const
    {
    return value.frequency > ngramToken.value.frequency || value.frequency ==
    ngramToken.value.frequency && ngram < ngramToken.ngram ;
    }
    bool operator>=( const NgramToken & ngramToken ) const
    {
    return value.frequency >= ngramToken.value.frequency || value.frequency ==
    ...
    bool operator==( const NgramToken & ngramToken ) const
    {
    return value.frequency == ngramToken.value.frequency && ngram ==
    ngramToken.ngram;
    }
    bool operator<( const NgramToken & ngramToken ) const
    {
   ...
   */
	...
	static int compareFunction (const void * a, const void * b)		/* sort by alphabet */
	{
		...
		/*utf8_string * x = *( ( utf8_string ** ) a );
		utf8_string * y = *( ( utf8_string ** ) b );
        return *x > *y ? 1 : *x == *y ? 0 : -1;
		*/
	}
#endif
  static int compareFunction(const void *a,
                             const void *b) /* sort by frequency and word */
  {
    int freq1 = (*((NgramToken **)a))->value.frequency;
    int freq2 = (*((NgramToken **)b))->value.frequency;
    return freq1 > freq2
               ? -1
               : freq1 == freq2 ? strcmp(((*(NgramToken **)a))->ngram.c_str(),
                                         (*((NgramToken **)b))->ngram.c_str())
                                : 1;
  }
  /**
   * constructor
   ...
   * for character ngram, the token will be a utf8_string of the character.
   */

  virtual void addToken(const utf8_string &token) = 0;
